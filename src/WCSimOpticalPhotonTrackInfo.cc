...
void WCSimOpticalPhotonTrackInfo::UserSteppingAction(const G4Step* aStep)
{
  ...
  if(!boundary){
    ...
    for( i=0;i<nprocesses;i++){
      //      std::cout << "Looking at process   " <<(*pv)[i]->GetProcessName() << " is an optical photon ? " << (track->GetParticleDefinition() == G4OpticalPhoton::OpticalPhotonDefinition()) << std::endl;
  ...
  if ( track->GetParticleDefinition() == G4OpticalPhoton::OpticalPhotonDefinition()) {
    ...
    // if we enable killing optical photons that 
    // scatter or reflect, do that here.
    if(killscatref){
      G4Track* nonconsttrack       = aStep->GetTrack();
